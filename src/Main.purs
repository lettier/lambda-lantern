    ...
    (\ framework -> do
      ...
      withWindow
        ...
        (\ window -> do
          ...
          font <- loadFont $ assetsPath <> "/fonts/gloria-hallelujah.ttf"
          ...
          mainMenuMusic <- getSound $ assetsPath <> "/music/big-eyes.ogg"
          levelMusic    <- getSound $ assetsPath <> "/music/night-in-the-castle.ogg"
          endingMusic   <- getSound $ assetsPath <> "/music/magic-in-the-garden.ogg"

          evilLaugh     <- getSound $ assetsPath <> "/sounds/evil-laugh.ogg"
          levelComplete <- getSound $ assetsPath <> "/sounds/level-complete.ogg"
          chicken       <- getSound $ assetsPath <> "/sounds/chicken.ogg"
          twinkle       <- getSound $ assetsPath <> "/sounds/twinkle.ogg"
          stoneDragging <- getSound $ assetsPath <> "/sounds/stone-dragging.ogg"
          doorOpen      <- getSound $ assetsPath <> "/sounds/door-open.ogg"
              ...
              (\ gameState ->
                case gameState of
                  MainMenu -> do
                    ...
                    logo <- loadModel window models $ assetsPath <> "/eggs/logo/logo"
                  ...
                  Level -> do
                    ...
                    room0 <- loadModel window models $ assetsPath <> "/eggs/room-0/room-0"
                    ...
                    mainCharacter <- loadModel window sceneRoot $ assetsPath <> "/eggs/main-character/main-character"
...
getAssetsPath :: Effect String
getAssetsPath = do
  path <- ifEmpty <$> getEnv "LAMBDA_LANTERN_ASSETS_PATH"
  pure $ toOsGenericFilename path
  where
    ifEmpty :: String -> String
    ifEmpty s
      | getStringLength s == 0 = "./assets"
      | otherwise = s
